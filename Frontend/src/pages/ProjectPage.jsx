                  ...
                  {cleanedEmployees.length > 0 ? (
                    cleanedEmployees.map((name, idx) => (
                      <tr key={`${name}-${idx}`}>
                        ...
                        <td>{name}</td>
                        <td>
                          {/* always show a delete button (server will enforce permissions) */}
                          <button className={styles.deleteButton} onClick={() => handleDeleteMember(name)} title="Remove member">🗑</button>
                          {currentMode === 'delete' && <button className={styles.removeButton} onClick={() => handleRemoveMember(idx, name)} style={{ marginLeft: 8 }}>-</button>}
                        </td>
                      </tr>
                    ))
                  ) : (

...
// Expose a function that can be invoked manually (and used by the cron schedule)
export default async function runNotificationJob() {
  try {
    ...
    console.error("Error in runNotificationJob:", err);
    throw err;
  }
}

// Also schedule the job to run every hour automatically
cron.schedule("0 * * * *", async () => {
  try {
    await runNotificationJob();
  } catch (err) {
    console.error("Scheduled notification job failed:", err);
  }
});
